...
public class LoadOrders extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		try{				
			String sql="select top "+ps+ " Id,Name,Tel,GymArea,"+
					"Place,SonPlace,SonArea,StartTime,OverTime,Date,Money,State,OrderTime "+
					"from BookTable where Id not in(select top "+ps*(pn-1)+" id from BookTable) order by id";
			String countSql="select count(*) from BookTable";
