...
/**
 * Servlet implementation class LoadType
 */
@WebServlet("/LoadType")
public class LoadType extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		try{				
			String sql="select top "+ps+ " Id,Type,Address,Picture,Price from MainF where Id not in(select top "+ps*(pn-1)+" id from MainF order by id) order by id";
			String countSql="select count(*) from MainF";
...
//	        System.out.println("获取的总条数为："+countString);
			// 获取列数  
...
//				            jsonObj.put("count", 11);
				            if(columnCount<10&&i==columnCount){
				            	jsonObj.put("msg","end");
				            }
				        }  
				        ls.add(jsonObj);
